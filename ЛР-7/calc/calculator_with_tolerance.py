...
def load_params(file_name: str) -> dict:
    ...
    :param file_name: The name of the file to load the parameters from.

    :return: A dictionary of parameters.
    """
    awaitable_params = {'tolerance', 'output'}
    params = {}

    try:
        with open(file_name, 'r') as file:  # file has to be in the same directory
            lines = file.readlines()
            for line in lines:
                key, value = line.strip().split('=')
                key = key.strip()
                value = value.strip()
                if key in awaitable_params:
                    params[key] = float(value) if '1' in value else value
                else:
                    raise ValueError(f'{key} is not a valid parameter')
    except (FileNotFoundError, PermissionError, TypeError, ValueError):
        print(f"File {file_name} does not exist, cannot be opened or contains invalid parameters.")
        print(f"Using default parameters.")
        params = {
            'tolerance': 1e-6,
            'output': 'default_calc.log'
        }
    except Exception as err:
        raise RuntimeError(f"Unexpected error in load_params: {err=}, {type(err)=}")
...
def setup_logger(file_name: str) -> logging.Logger:
    ...
    try:
        ...
        try:
            ...
            raise PermissionError(f"Cannot create file {file_name}. Permission denied.")
        else:
...
def data_manager(prompt: str, use_params: bool = False) -> tuple[str, list[str], float, str]:
    operator, *numbers, tolerance = prompt_parser(prompt)                                       # Parsing the input

    if use_params:
        PARAMS = load_params(os.path.dirname(os.path.realpath(__file__)) + '/' + 'config.ini')  # Reading parameters
        try:
            tolerance = PARAMS['tolerance']                                                         # This is unreliable
            file = PARAMS['output']
        except KeyError:
            raise KeyError(f"Parameter {key} does not exist in config.ini")
    else:
        file='second_calc_log.log'

    return operator, numbers, tolerance, file
...
def decoration_logger(calc: callable) -> callable:
    ...
    """
    def wrapper(prompt: str, use_params: bool = False) -> float | int | None:
        """
        ...
        """
        operator, numbers, tolerance, file = data_manager(prompt, use_params=use_params)

        logger = setup_logger(file_name=file)                                   # Setting up logger
...
if __name__ == "__main__":
    ...
    try:
        use_default = input("Use default parameters? (y/n): ").lower() == 'y'   # Returns True if user enters 'y'
    ...
    print(calculate(prompt, use_params=use_default))
