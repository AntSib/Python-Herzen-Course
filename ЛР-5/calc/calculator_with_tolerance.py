...
logging.basicConfig(filename='second_calc_log.log', level=logging.INFO)   # Basic setup for logger


def greet():
    """Print all available operators"""
    print("Choose your operator: ", end='')
    for option in awailable_operators:
        print(f'"{option}"', end='  ')
    print()


def prompt_parser(prompt: str) -> tuple:
    """
    ...
    """
    operator, *numbers, tolerance = prompt.split(' ')

    return operator, numbers, float(tolerance)
...
def decoration_logger(func: callable) -> callable:
    ...
    def wrapper(prompt: str) -> float:
        ...
        logger.info(f"{operator},{",".join(numbers[0])},{tolerance}={res}") # logging data
